...
The focus so far has been on analysing your data either with a statistical test or plotting your data to look at relationships, differences or patterns. But in biology and ecology, we often ask questions related to the distribution and contribution of different species to a community. We have seen with the ordination methods that we can reduced the dimensional of our species data into (relatively) easily interpretable figures where we can see sites are grouped together or that species close in ordination space are likely to co-occur. However, there times when it will be important to calculate a **diversity index** for a sample, habitat or ecosystem.
...
We can now undertake a statistical analysis on to see whether there are significant differences in Margalef's index among the different management types. This can be done using a linear model with continuous or categorical variables. 
...
Diversity indices are important measures of community composition. They are good for understanding whether communities are different or similar but they need to be used with caution. It is best to combine the use of diversity indices with multivariate analyses to have a deeper interpretation of the diversity indices.
